...
export function useCurrentTab() {
  ...
  const loadTabInfo = async () => {
    ...
    try {
      ...
      if (tabResponse.error === 'TAB_PERMISSION_REQUIRED') {
        setNeedsPermission(true);
        return;
      }
    } catch (error) {
      // Avoid noisy logs for expected permission cases; surface other failures.
  ...
  return { tabInfo, isLoading, needsPermission, reload: loadTabInfo };
}
